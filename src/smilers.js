...
class PopUp extends Phaser.GameObjects.Group {
    constructor(scene) {
        ...
        this.textBody = scene.add.text(GAME.SIZE/2, 100, "", {
            fontFamily: 'Arial',
            fontSize: '32px',
            color:  '#000000',
            wordWrap: {
...
class Butterfly extends Phaser.Physics.Arcade.Sprite {
    constructor(scene, x, y) {
        ...
        getRandomInt(0, 1) === 1 ? this.anims.play('left') : this.anims.play('right');
        return this;
    }
    ...
    update() {
        if (this.body) {
            ...
            if (rand === 1) {
                this.setVelocity(getRandomInt(-3*PLAYER_SPEED, 3*PLAYER_SPEED), getRandomInt(-GRAVITY, GRAVITY));
            } else if (rand > 43) {
...
class Flower extends Phaser.Physics.Arcade.Sprite {
    constructor(scene, x, y, texture) {
        ...
        this.anims.create({
            key: 'sway2', 
            frameRate: FRAMERATE/4,
            frames: this.anims.generateFrameNumbers(texture, { frames: [2, 3, 0, 1] }),
            repeat: -1
        });
...
class DrawvidDotCom extends Phaser.Scene {
    ...
    preload() {
		...
		this.load.spritesheet('pinkButterfly', 'assets/butterfly-pink.png', { frameWidth: 16, frameHeight: 16, margin: 0, spacing: 0 });
    }
    ...
    create() {
        ...
        this.player = new Smiler(this, GAME.SIZE/2, GAME.SIZE/2 + PLAYER_SIZE * GLOBAL_SCALE);
    ...
    addButtons() {
        let code = this.physics.add.image(GAME.SIZE/2 - PLAYER_SIZE*2*GLOBAL_SCALE - PLAYER_SIZE, GAME.SIZE/3, 'code').setInteractive();
        ...
        code.setDepth(11);
        let about = this.physics.add.image(GAME.SIZE/2, GAME.SIZE/3, 'about').setInteractive();
    ...
    spawnClouds() {
        for (let i = 0; i < getRandomInt(MIN_CLOUDS, MAX_CLOUDS); i++) {
            ...
            let cloud = this.physics.add.sprite(getRandomInt(-PLAYER_SIZE*GLOBAL_SCALE, GAME.SIZE+PLAYER_SIZE*GLOBAL_SCALE), yPos, 'cloud');
    ...
    spawnBackFlowers() {
        let flower1 = new Flower(this, GAME.SIZE/4 - PLAYER_SIZE*GLOBAL_SCALE, GROUNDY - 2*PLAYER_SIZE, 'pflower');
        let flower2 = new Flower(this, GAME.SIZE/2 + PLAYER_SIZE*GLOBAL_SCALE, GROUNDY - 2*PLAYER_SIZE, 'oflower');
        flower1.anims.play('sway1', true);
        flower2.anims.play('sway1', true);
    }

    spawnFrontFlowers() {
        let flower1 = new Flower(this, GAME.SIZE/2 - PLAYER_SIZE*GLOBAL_SCALE, GROUNDY - 3*PLAYER_SIZE/2, 'bflower');
        let flower2 = new Flower(this, GAME.SIZE - PLAYER_SIZE*GLOBAL_SCALE, GROUNDY - 3*PLAYER_SIZE/2, 'pflower');
    ...
    getFrame(yPos, yMax) {
        return yPos < yMax/4 ? 0 : yPos < yMax/2 ? 1 : yPos < yMax*3/4 ? 2 : 3;
    }
    ...
    playerMovementHandler() {
        if ((this.controls.up.isDown || this.controls.space.isDown) && this.player.body.velocity.y === 0 && this.player.y >= GROUNDY - PLAYER_SIZE*GLOBAL_SCALE - GLOBAL_SCALE) {
    ...
    playerMobileMovementHandler() {
        ...
        if ((pointer.isDown)) {
            ...
            if (touchWorldPoint.y <= GROUNDY-PLAYER_SIZE*GLOBAL_SCALE && this.player.body.velocity.y === 0 && this.player.y >= GROUNDY - PLAYER_SIZE*GLOBAL_SCALE - GLOBAL_SCALE) {
                this.player.setVelocityY(JUMP_VELOCITY);
            } else {
                if (touchWorldPoint.x > this.player.body.position.x + PLAYER_SIZE*GLOBAL_SCALE/2) {
                    this.player.setVelocityX(this.player.speed);
                } else if (touchWorldPoint.x < this.player.body.position.x - PLAYER_SIZE*GLOBAL_SCALE/2) {
                    this.player.setVelocityX(-this.player.speed);
                } else if (this.player.body.velocity.y === 0 && this.player.y >= GROUNDY - PLAYER_SIZE*GLOBAL_SCALE - GLOBAL_SCALE) {
                    this.player.setVelocityY(JUMP_VELOCITY);
                }
            }
        }
